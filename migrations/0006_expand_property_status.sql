-- Migration 0006: Expand status_publish CHECK constraint to include 'sold' and 'archived'
-- SQLite tidak support ALTER COLUMN — perlu recreate table dengan pragma defer FK off
-- 'archived' = soft-delete: properti disembunyikan dari publik, data tidak dihapus
-- 'sold'     = terjual, tidak tampil di pencarian publik (publik endpoint filter published only)
