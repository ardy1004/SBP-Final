...
- **⚠️ PALING PENTING — paket Cloudflare FREE = 10 ms CPU per request.** Ini batas **per-request**, BUKAN startup dan BUKAN ukuran bundle. `renderToString` React di project ini tidak muat di 10 ms, sehingga sebagian request SSR dibunuh runtime dengan **Error 1102 "Worker exceeded resource limits"** → halaman blank putih. Gejalanya **berkedip**: URL yang sama kadang 200, kadang 503, jadi satu curl bisa memberi lampu hijau palsu. Gunakan `npm run smoke` (320 probe), jangan pernah cek sekali jalan.
  - **JANGAN salah diagnosis lagi.** Pada 2026-07-25 ini sempat dikira batas *startup*/bundle dan berjam-jam habis mengecilkan bundle (hasilnya nyata tapi tidak menyelesaikan). Uji pembeda yang menutup perdebatan (2026-07-26, konkurensi 12, tanpa cache): `/robots.txt` (non-Worker) gagal **0%**, `/admin/login` (skeleton nyaris kosong) **13%**, `/privacy` (statis, tanpa loader) **73%**, `/` **85%**. Semua di Worker yang sama. Yang menentukan adalah **berat render**, bukan startup — kalau startup, semuanya akan gagal merata.
  - Konsekuensi praktis: **tidak ada optimasi per-halaman yang menyelamatkan ini** — `/privacy` saja sudah gagal 73%. Mitigasi yang berhasil = cache edge (di bawah). Solusi tuntas = tidak merender di Worker sama sekali (prerender/SSG) atau Workers Paid.
