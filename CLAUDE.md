...
- **Gate sebelum deploy**: `npm run check:bundle` (anggaran permukaan evaluasi eager, wajib exit 0) lalu `npm run smoke -- https://salambumi.xyz [--cached]`. Mode `--cached` mengukur pengalaman pengguna nyata; mode default mengukur skenario terburuk tanpa cache. **Laporkan keduanya — satu angka saja menyesatkan.**
