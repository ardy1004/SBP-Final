...
- **Gate sebelum deploy**: `npm run typecheck` → `npm run check:bundle` (anggaran permukaan evaluasi eager) → `npm run smoke -- https://salambumi.xyz [--cached]`. Ketiganya wajib exit 0. Mode `--cached` mengukur pengalaman pengguna nyata; mode default mengukur skenario terburuk tanpa cache. **Laporkan keduanya — satu angka saja menyesatkan.**
