...
import { SlidersHorizontal, Grid3X3, List, X, ChevronDown, RotateCcw, MapPin } from 'lucide-react';
import { PROPERTIES, LOCATION_HIERARCHY, formatRupiah, type Property } from '../data/mockData';
...
const HARGA_RANGES_JUAL = [
  { label: 'Semua Harga', min: 0, max: Infinity },
  { label: '< 500 Jt', min: 0, max: 500000000 },
  { label: '500 Jt – 1 M', min: 500000000, max: 1000000000 },
  { label: '1 M – 2 M', min: 1000000000, max: 2000000000 },
  { label: '2 M – 3 M', min: 2000000000, max: 3000000000 },
  { label: '3 M – 5 M', min: 3000000000, max: 5000000000 },
  { label: '5 M – 10 M', min: 5000000000, max: 10000000000 },
  { label: '> 10 M', min: 10000000000, max: Infinity },
];
...
export default function PropertiesPage() {
  ...
  // Filters state
  const [tujuan, setTujuan] = useState<string>(searchParams.get('tujuan') || 'semua');
  ...
  const kabList = Object.keys(loc);
  const kecList = kabupaten
    ? Object.keys((loc[kabupaten as keyof typeof loc] as Record<string, Record<string, string[]>>) || {})
    : [];
  ...
  const filtered = useMemo(() => {
    let list = [...PROPERTIES];
    if (tujuan !== 'semua') list = list.filter(p => p.tujuan === tujuan || p.tujuan === 'dijual_disewa');
    if (selectedJenis.length > 0) list = list.filter(p => selectedJenis.includes(p.jenis.toLowerCase()));
    if (hargaRange > 0) {
      const r = HARGA_RANGES_JUAL[hargaRange];
      list = list.filter(p => p.harga >= r.min && p.harga <= r.max);
    }
    if (kabupaten) list = list.filter(p => p.kabupaten === kabupaten);
    if (kecamatan) list = list.filter(p => p.kecamatan === kecamatan);
    ...
    return list;
  }, [tujuan, selectedJenis, hargaRange, kabupaten, kecamatan, sort]);

  const totalPages = Math.ceil(filtered.length / PER_PAGE);
  const paginated = filtered.slice((page - 1) * PER_PAGE, page * PER_PAGE);

  const activeChips = [
    tujuan !== 'semua' ? { label: tujuan === 'dijual' ? 'Dijual' : 'Disewa', clear: () => setTujuan('semua') } : null,
    ...selectedJenis.map(j => ({ label: j, clear: () => toggleJenis(j) })),
    hargaRange > 0 ? { label: HARGA_RANGES_JUAL[hargaRange].label, clear: () => setHargaRange(0) } : null,
    kabupaten ? { label: kabupaten, clear: () => { setKabupaten(''); setKecamatan(''); } } : null,
    kecamatan ? { label: kecamatan, clear: () => setKecamatan('') } : null,
    ...
    <div className="p-5">
      ...
      <div className="mb-5">
        ...
        <div className="relative">
          <select value={hargaRange} onChange={e => { setHargaRange(Number(e.target.value)); setPage(1); }} className={selectClass}>
            {HARGA_RANGES_JUAL.map((r, i) => <option key={i} value={i}>{r.label}</option>)}
          </select>
      ...
      <div className="mb-5">
        ...
        <div className="space-y-2">
          <div className="relative">
            <select value={kabupaten} onChange={e => { setKabupaten(e.target.value); setKecamatan(''); setPage(1); }} className={selectClass}>
          ...
          {kabupaten && (
            <div className="relative">
              <select value={kecamatan} onChange={e => { setKecamatan(e.target.value); setPage(1); }} className={selectClass}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm text-[#64748B]">
                  Menampilkan <span className="font-bold text-[#0F172A]">{paginated.length}</span> dari <span className="font-bold text-[#0F172A]">{filtered.length}</span> properti
                </p>
              </div>
              <div className="flex items-center gap-2">
                <div className="relative">
                  <select value={sort} onChange={e => setSort(e.target.value)} className="border border-gray-200 rounded-xl px-3 py-2 text-sm appearance-none bg-white pr-8 focus:outline-none focus:ring-2 focus:ring-[#1565C0]">
            ...
            {paginated.length === 0 ? (
              <div className="text-center py-20">
                ...
                <button onClick={resetFilters} className="px-6 py-2.5 rounded-xl text-white font-semibold" style={{ background: 'linear-gradient(135deg, #1565C0 0%, #29B6F6 100%)' }}>
                  Reset Filter
                </button>
              </div>
            ) : viewMode === 'grid' ? (
                ...
                {paginated.map(p => (
                  <div key={p.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex">
                    <div className="relative w-48 sm:w-64 flex-shrink-0">
                      <img src={p.images[0]} alt={p.title} className="w-full h-full object-cover" style={{ minHeight: 160 }} onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1624204386084-dd8c05e32226?w=400&q=80'; }} />
                    </div>
                    <div className="p-4 flex-1">
                      ...
                      <div className="flex items-center gap-1 text-[#64748B] text-xs mb-2"><MapPin size={10} />{p.kecamatan}, {p.kabupaten}</div>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute top-0 left-0 h-full w-80 bg-white overflow-y-auto">
            ...
            <SidebarContent />
            <div className="p-4">
              <button onClick={() => setSidebarOpen(false)} className="w-full py-3 rounded-xl text-white font-semibold" style={{ background: 'linear-gradient(135deg, #1565C0 0%, #29B6F6 100%)' }}>
                Terapkan Filter
              </button>
            </div>
          </div>
